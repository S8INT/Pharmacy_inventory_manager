...
def add_patient_from_input(patients_name_entry, patients_address_entry, patients_phone_number_entry,
                           patients_allergies_entry, patients_treeview):
...
def clear_input_fields(patients_name_entry, patients_address_entry, patients_phone_number_entry,
                       patients_allergies_entry):
...
def patients_window():
    ...
    add_patient_button = tk.Button(control_frame1, text="Add Patient", command=lambda: add_patient_from_input(
        patients_name_entry, patients_address_entry,
        patients_phone_number_entry,
    ...
    delete_patient_button = tk.Button(control_frame1, text="Delete Patient",
                                      command=lambda: delete_patient(patient_treeview))
    delete_patient_button.grid(row=0, column=5, padx=5, pady=5)

    # Create the save patient button
    save_patient_button = tk.Button(control_frame1, text="clear",
                                    command=lambda: clear_input_fields(patients_name_entry,
                                                                       patients_address_entry,
                                                                       patients_phone_number_entry,
                                                                       patients_allergies_entry))
...
def add_prescription(patient_id, medicine_id, dosage, frequency, start_date, end_date, prescription_treeview):
    try:
        ...
        cursor.execute("""INSERT INTO prescriptions (patient_id, medicine_id, dosage,
         frequency, start_date, end_date)
                        VALUES (?, ?, ?, ?, ?, ?)""", (patient_id,
                                                       medicine_id, dosage, frequency, start_date, end_date))
        conn.commit()
...
def delete_selected_prescription(prescription_treeview):
    try:
        ...
        # Get the ID from the item's values
        selected_prescription_id = prescription_treeview.item(selected_item, 'values')[0]

        if selected_prescription_id:  # Check if a prescription is selected
            # Confirmation prompt before deletion
            confirmation = tk.messagebox.askquestion("Confirm Delete", "you want to delete prescription?")
            ...
            if confirmation == 'yes':
                try:
                    ...
                    tk.messagebox.showerror("Error",
                                            "An error occurred while deleting the prescription from the database.")
        else:
...
def prescription_window():
    ...
    delete_prescription_button = tk.Button(middle_frame, text="Delete Prescription",
                                           command=lambda: delete_selected_prescription(prescription_treeview))
...
# ======================================================================================================================
def calculate_bill(patient_id, prescription_id, total_cost_entry):
    try:
        # Fetch the prescription details from the database
        cursor.execute("SELECT * FROM prescriptions WHERE id = ? AND patient_id = ?", (prescription_id, patient_id))
        ...
        if prescription is not None:
            ...
            if medicine is not None:
                ...
                if patient is not None:
                    ...
                    total_cost_entry.delete(0, tk.END)  # Clear any previous value
                    total_cost_entry.insert(0, str(total_cost))  # Update total cost entry field
                    return total_cost, patient, medicine, prescription
                else:
...
def display_bill_details(bill_id, bill_treeview):
    try:
        ...
        if bill is not None:
            # insert the bill details into the treeview
            bill_treeview.insert('', 'end', values=bill)
        else:
...
def save_bill(patient_id, prescription_id, total_cost, billing_treeview):
    try:
        # Insert new bill data into the database
        cursor.execute("""INSERT INTO bills (patient_id, prescription_id, total_cost)
                        VALUES (?, ?, ?)""", (patient_id, prescription_id, total_cost))
        conn.commit()
        # Update the display frame after adding the bill
        update_bill_display(billing_treeview)
    except sqlite3.Error as e:
        print(f"An error occurred: {e}")
        tk.messagebox.showerror("Error", "An error occurred while saving the bill to the database.")
...
def billing_window():
    ...
    calculate_bill_button = tk.Button(bmiddle_frame, text="Calculate Bill", command=lambda: calculate_bill(
        patient_id_entry.get(), prescription_id_entry.get(), total_cost_entry))
    ...
    show_bill_button = tk.Button(bmiddle_frame, text="Show", command=lambda: display_bill_details)
    show_bill_button.grid(row=3, column=2, padx=5, pady=5)
    ...
    # Create the billing list treeview
    columns = ["ID", "Patient ID", "Prescription ID", "Total Cost"]
    billing_treeview = ttk.Treeview(billing_list_frame, columns=columns,
                                    show="headings", style="mystyle.Treeview")
    for col in columns:
        billing_treeview.heading(col, text=col)
        billing_treeview.column(col, width=100, anchor="center")
    billing_treeview.pack(side="left", fill="both", expand=True)
    ...
    # Create the scrollbar for the treeview
    scrollbar = ttk.Scrollbar(billing_list_frame, orient="vertical", command=billing_treeview.yview)
    scrollbar.pack(side="right", fill="y")
    billing_treeview.configure(yscrollcommand=scrollbar.set)

    # Populate the treeview with bills


def update_bill_display(billing_treeview):
    try:
        # Clear existing rows in the Treeview
        for row in billing_treeview.get_children():
            billing_treeview.delete(row)
        # Fetch all bills from the database
        cursor.execute("SELECT * FROM bills")
        bills = cursor.fetchall()
        # Insert each bill into the Treeview
        for bill in bills:
            billing_treeview.insert('', 'end', values=bill)
    except sqlite3.Error as e:
        print(f"An error occurred: {e}")
        tk.messagebox.showerror("Error", "An error occurred while fetching bills from the database.")
...
def reports_window():
    ...
    generate_report_button.grid(row=0, column=2, padx=5, pady=5)
...
# hide main window until user logs in successfully
window.withdraw()

# create a navigation bar
nav_bar = tk.Menu(window)
window.config(menu=nav_bar)

# create a file menu
file_menu = tk.Menu(nav_bar, tearoff=0)
nav_bar.add_cascade(label="File", menu=file_menu)
file_menu.add_command(label="Exit", command=exit_window)

# create a help menu
help_menu = tk.Menu(nav_bar, tearoff=0)
nav_bar.add_cascade(label="Help", menu=help_menu)
help_menu.add_command(label="About")

# create edit menu
edit_menu = tk.Menu(nav_bar, tearoff=0)
nav_bar.add_cascade(label="Edit", menu=edit_menu)
edit_menu.add_command(label="Edit")

# create a view menu
view_menu = tk.Menu(nav_bar, tearoff=0)
...
if __name__ == "__main__":
    # create login window
    login_window = create_login_window(window)

    # show login window and start main loop
